...
function KatanaItem::onAdd(%this, %obj)
{
...
function KatanaImage::onMount(%image, %player, %slot)
{
...
function KatanaImage::onWindUp(%image, %player)
{
...
function KatanaImage::onFire(%image, %player)
{
	cancel(%player.windUpSchedule);
	%player.swingType = getRandom(1, 2);
...
function KatanaImage::onMeleeHit(%image, %player, %object, %position, %normal)
{
	if (!isObject(%object))
		return;
